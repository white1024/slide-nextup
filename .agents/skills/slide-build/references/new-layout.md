...
- An image slot may declare `fit`: `cover` (the default) fills the box and crops what does not fit, for photos; `contain` shows the whole picture and leaves bands, for diagrams and charts. Declare it in layout.json rather than overriding `object-fit` in layout.css; QA and the gallery warn when a cover picture's ratio is far from its box's (cover would crop more than a quarter of it).
- CSS ownership (`pnpm theme:lint`): layout.css sets geometry only, theme.css appearance only. A decorative pseudo-element (`::before` / `::after`) may carry `content: ""` on either side, for the rule that trails an eyebrow or the dot before a label, so such a line needs no shape element of its own; text, `attr()` and `counter()` in `content` stay forbidden because the editor cannot reach generated text. QA measures DOM boxes, so a pseudo-element is decoration, never layout.
...
- Every element carries a `data-role`, from the existing vocabulary so the theme can paint it:
  ...
  - a role of your own is allowed (the theme has to paint it); its font floor is that of its longest known prefix (`eyebrow-accent-2` → `eyebrow-accent`, 20px), else the content floor of 32px. For colour variants of one role use `data-tone` instead of a new role (below).
...
- Hints and the unit rule: a slot's `hint` states the count the box really holds, in the grammar `pnpm layout:gallery --capacity` parses: `up to 24 characters`, `4 to 12 characters` (a range keeps its larger end), `up to 8 characters per line`, `1 to 10 characters per cell`, `one line`, `up to 2 lines`, `3 to 5 items, one line each`; numbers as digits or the words one to ten and twelve, digits above ten. Character counts are full-width units: a CJK glyph counts 1, a Latin letter or digit half, so `up to 24 characters` is about 48 Latin letters. `up to 7 characters per line, up to 2 lines` is more useful than `up to 10 characters`, because CJK text wraps anywhere. An item count (`3 to 5 items`, `2 to 3 supporting points`, and their Chinese equivalents — a word or two may sit between the number and the unit) is checked against the box the way the items are really laid out: a list as items × lines each plus the margin between items, a flow of chips (the `flow` roles) as one chip's width, how many fit in a row and how many rows the box holds. An element with no list items — a table, a paragraph — is only checked on characters and lines. Both are the worst case the hint promises, so `2 to 3 items, up to 2 lines each` fails a box that holds only two such items even when the sample fits; shrink the hint to what the box guarantees rather than the box to what the hint says. The gallery prints the measured characters per line and lines of every text box, and a hint that promises more than the box holds fails; `pnpm theme:qa` runs the same check over every slot and reports it as a `hint-capacity` warning, which fails a theme pack. Do not use "line(s)", "character(s)", "item(s)", "row(s)", "each" or "per" loosely where they are not a limit (write "a short phrase", not "a line of text"); "headline" and "outline" are fine, the parser needs a number right before `line`.
...
- An inverted slide sets `data-tone="inverse"` on the section. `data-tone` is also how one role gets variants without a new role: the layout sets `data-tone="second"` on an element and the theme styles `[data-role="eyebrow-accent"][data-tone="second"]` (technical-brief's teal, emerald and amber pips on its three cards). A theme lists the tones it styles in theme.json `tones`; `theme:check` warns when a layout asks for a tone the theme has no rule for, because that element would silently keep the role's plain look.
