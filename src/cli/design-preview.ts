...
import { loadLayout, loadTheme } from '../render/assets.ts'
...
if (!target || !themeId) {
  console.error('Usage: pnpm design:preview <story.md> --theme <id> [--layout cover] [-o dir]')
  process.exit(2)
}
...
if (loaded.hasErrors || !loaded.story) {
  console.log('the story file failed its checks; fix it before previewing')
  process.exit(1)
}
const story = loaded.story
const first = story.slides[0]
if (!first) {
  console.log('the story has no slides')
  process.exit(1)
}
...
const outDir = resolve(opt('-o') ?? join('artifacts', 'design', deckIdFromStoryPath(storyFile)))
