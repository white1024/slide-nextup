...
import { listThemeIds } from '../render/assets.ts'

const args = process.argv.slice(2)
if (args.includes('--help') || args.includes('-h')) {
  console.log(
    [
      '用法：pnpm theme:qa [--theme <id>] [layout…]',
...
const only = args.filter((a, i) => !a.startsWith('-') && i !== themeIndex + 1)
...
for (const id of themes) {
  const report = await runThemeQa(id, { browser, only })
